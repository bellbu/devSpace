...
$(function () {
	...
	$(document).off().on('click',".man,.woman", function(){
  		...
  		keyNo = this.dataset.val;

  		var sexClass = $(this).attr('class');
...
//   		 	}
  			...
  			$(this).dblclick(function(){
		 	 	...
		 	 	$('#sex').val(data.sex).trigger('change',true);
	  	  		...
	  	  		$('#entDiv').val(data.entDiv).trigger('change',true);
	  	  		$('#roomNo').val(data.roomNo);
	  	  		$('#payLockNo').val(data.payLockNo);
	  	  		
	  	  		$('#keyAddress').val(data.keyAddress);
	  	  		$('#upjangCode').val(data.upjangCode);
	  	  		$('#saleDate').val(data.saleDate);
	  	  		$("#menuCode").val(data.menuCode).trigger('change',true);
	  	  		$('#memType').val(data.memType);
	  			$('#spaFareType').val(data.spaFareType);
	  			$('#saleAmt').val(data.saleAmt);
	  			$('#possible').val(data.possible);
	  			$('#all').val(data.all);
...
function f_InitTopView() {
	...
	$("#menuCode").empty();
	 ...
	 //entDiv 임시
     $("#entDiv").setSelectDataSet('${cmmnObj.BT009}', "chk");
     $("#relativeType").setSelectDataSet('${cmmnObj.BT001}', "chk");
     $("#memType").setSelectDataSet('${cmmnObj.CR034}', "chk");
	 $("#vMemType").setSelectDataSet('${cmmnObj.CR034}','chk');
     $("#spaFareType").setSelectDataSet('${cmmnObj.SP002}', "chk");
 ...
 function f_setEvent() {
	 console.log("function f_setEvent()");
	    $("#gr_Partner").createGridEvent(
	          ''   // create
	          ,{   // read
	             onClickEvent:'f_PartnerBind'
	          } 
	          ,''   // update
	          );
	    /*	
...
function f_BytesHandler(obj) {
	 console.log("function f_BytesHandler(obj)");
...
function f_FormClear(formId) {
	 console.log("function f_FormClear(formId)");
...
function f_PopupsGuestName() {
	...
	if(houseCk)
	{
		PopType = "RetrieveHouseMemberListPop";
		BindData = [
...
function f_Copy2() {
	...
	var addObject = new Object();
	addObject.keyNo = keyNo;
// 	addObject.guestName = "11";
// 	addObject.memType = "11";
// 	addObject.relativeType = "11";
// 	addObject.roomNo = "11";
// 	addObject.sex = "11";
// 	addObject.payKeyNo = "11";
// 	addObject.menuName = "11";
// 	addObject.saleAmt = "11";
// 	addObject.remarks = "11";
// 	addObject.spaYn = "11";
// 	addObject.gymYn = "11";
// 	addObject.enterTime = "11";
// 	addObject.exitTime = "11";
// 	addObject.paidYn = "11";
// 	addObject.keyAddress = "11";
// 	addObject.upjangCode = "11";
// 	addObject.saleDate = "11";
// 	addObject.menuCode = "11";
	addObject.type = "add"
	$("#gr_Partner").GridAdd(addObject);
...
function f_Del2(){
	...
	for(var i=0; i < gridData.length ; i++){
		if(gridData[i].keyNo == keyNo){
			delRowKey = gridData[i].rowKey;
		} 
	}
	
	grid.removeRow(delRowKey);
...
function f_GetGuestInfo(guestNo)
{
        	 ...
        	 <div class="col-md-12">
		            <div class="card mb-12">
		                <div class="card-body">
		                    ...
		                    <input type="" id="stat"name="type" value="" />
							<form id="insertForm" method="post" onsubmit="return false;"  method="post" action="/btm/SpaLockerSalesRegistration_Control.do">
							<input type="" id="imgPath"name="imgPath" value="" />
							<input type="" id="type"name="type" value="" />
							<input type="" id="copy"name="copy" value="N" />
							<input type="" id="keyAddress"name="keyAddress" value="" />
		                	<input type="" id="upjangCode"name="upjangCode" value="" />
		                    <input type="" id="saleDate"name="saleDate" value="" />
